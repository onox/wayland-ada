   ...
   procedure Create_Wayland_Body_File is
      ...
      procedure Generate_Manually_Edited_Code (Protocol_Name : String) is
         ...
         begin
            ...
            Put_Line (File, "      Descriptors : File_Descriptor_Array := (1 .. 0 => 0)) return Events_Status_Array");
            Put_Line (File, "   is");
            Put_Line (File, "      Descriptor : constant Wayland.File_Descriptor :=");
            Put_Line (File, "        Wayland.API.Display_Get_File_Descriptor (Object.Proxy);");
            Put_Line (File, "");
            Put_Line (File, "      Bits : constant Wayland.Posix.Event_Bits := (Input => True, others => False);");
            Put_Line (File, "");
            Put_Line (File, "      function Get_Events (Descriptors : File_Descriptor_Array)");
            Put_Line (File, "        return Wayland.Posix.Requested_Event_Array is");
            Put_Line (File, "      begin");
            Put_Line (File, "         return Result : Wayland.Posix.Requested_Event_Array (Descriptors'Range) do");
            Put_Line (File, "            for Index in Result'Range loop");
            Put_Line (File, "               Result (Index) := (FD => Descriptors (Index), Events => Bits);");
            ...
            Put_Line (File, "   begin");
            Put_Line (File, "      return Result : Events_Status_Array (Events'Range) do");
            Put_Line (File, "         for Index in Result'Range loop");
            Put_Line (File, "            if Events (Index).Is_Success then");
            Put_Line (File, "               Result (Index) := (if Events (Index).Events.Input then Has_Events else No_Events);");
            Put_Line (File, "            else");
            Put_Line (File, "               Result (Index) := Error;");
            Put_Line (File, "            end if;");
            Put_Line (File, "         end loop;");
            Put_Line (File, "      end return;");
            ...
            Put_Line (File, "");
            Put_Line (File, "            declare");
            Put_Line (File, "               Result : constant Wayland.Posix.Returned_Event_Array := Wayland.Posix.Poll");
            Put_Line (File, "                 ((1 => (FD     => Descriptor,");
            Put_Line (File, "                         Events => (Output => True, others => False))),");
            Put_Line (File, "                  Timeout => Duration'Last);");
            Put_Line (File, "            begin");
            Put_Line (File, "               if Result'Length = 0 or else not Result (Result'First).Is_Success then");
            Put_Line (File, "                  return (Is_Success => False);");
            Put_Line (File, "               end if;");
            Put_Line (File, "            end;");
            Put_Line (File, "         end;");
         ...
         is
            ...
            begin
               ...
               Put_Line (File, "     (Object  : Pointer_Gestures_V1;");
               Put_Line (File, "      Pointer : Client.Pointer'Class;");
               Put_Line (File, "      Gesture : in out Pointer_Gesture_" & Name & "_V1'Class) is");
               Put_Line (File, "   begin");
               Put_Line (File, "      Gesture.Proxy := Thin.Pointer_Gestures_V1_Get_" & Name & "_Gesture");
               Put_Line (File, "        (Object.Proxy, Thin_Client.Pointer_Ptr (Pointer.Get_Proxy));");
               Put_Line (File, "   end Get_" & Name & "_Gesture;");
            end Print_Constructor;
         ...
         begin
            ...
            if Name = "Pointer_Gesture_Swipe_V1" then
               ...
               Put_Line (File, "      procedure Internal_Begin");
               Put_Line (File, "        (Data                     : Void_Ptr;");
               Put_Line (File, "         Pointer_Gesture_Hold_V1 : Thin.Pointer_Gesture_Hold_V1_Ptr;");
               Put_Line (File, "         Serial                   : Unsigned_32;");
               Put_Line (File, "         Time                     : Unsigned_32;");
               Put_Line (File, "         Surface                  : Thin_Client.Surface_Ptr;");
               Put_Line (File, "         Fingers                  : Unsigned_32)");
