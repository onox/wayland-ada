...
package Px is
   ...
   type Prot_FLag_T is new Unsigned_32;
   ...
   -- Page can be read.
   PROT_READ : constant Prot_FLag_T := 16#1#;

   -- Page can be written.
   PROT_WRITE : constant Prot_FLag_T := 16#2#;

   -- Page can be executed.
   PROT_EXEC : constant Prot_FLag_T := 16#4#;

   -- Page can not be accessed.
   PROT_NONE : constant Prot_FLag_T := 16#0#;

   -- Extend change to start of growsdown vma (mprotect only).
   PROT_GROWSDOWN : constant Prot_FLag_T := 16#01000000#;

   -- Extend change to start of growsup vma (mprotect only).
   PROT_GROWSUP : constant Prot_FLag_T := 16#02000000#;
   ...
   procedure Write (File : File_T; Bytes : Byte_Array_T) with
      Global => null,
      Pre    => File.Is_Open;
   ...
   procedure Memory_Map
     (File    : in File_T;
      ...
      Flags   : int;
      Offset  : Offset_T;
      Memory_Map : in out Memory_Map_T) with
     Global => null,
   ...
   function Size (Status : Status_T) return Offset_T with
      Global => null,
      Pre    => Status.Is_Valid;
   ...
   type Memory_Map_T is tagged limited private;

   function Has_Mapping (Map : Memory_Map_T) return Boolean with
     Global => null;

   function Mapping (Map : Memory_Map_T) return Void_Ptr with
     Global => null,
     Pre    => Map.Has_Mapping;
...
private
   ...
   package Px_Thin is
      ...
      function Mmap
        (Addr   : Void_Ptr;
         ...
         Flags  : int;
         Fd     : int;
         Offset : Offset_T) return Void_Ptr with
         Import        => True,
         Convention    => C,
         External_Name => "mmap";

      function Munmap (Addr : Void_Ptr; Length : Size_T) return int with
         Import        => True,
         Convention    => C,
   ...
   function Size
     (Status : Status_T) return Offset_T is
     (Status.My_Status.Size);

   function Block_Size
     (Status : Status_T) return Block_Size_T is
   ...
   function Last_Access_Time
     (Status : Status_T) return Time_T is
   ...
   type Memory_Map_T is tagged limited record
